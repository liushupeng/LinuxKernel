...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1)
        strncpy(filename, argv[1], MAXNAME);
    else {
    ...
    unsigned long filesize = lseek(fd, 0LL, SEEK_END);
    ...
    while (!done) {
        ...
        if (pageincr == 0LL)
            pageincr = incmax;
        else if (pageincr < incmin)
            pageincr = incmin;
        else if (pageincr > incmax)
            pageincr = incmax;
        ...
        for (i = 0; i < BUFHIGH; i++) {
            ...
            case 1: // 'byte' format
                ...
                for (j = 0; j < BUFWIDE; j++)
                    linelen += sprintf(outline + linelen, "%02X ", bp[j]);
                break;

            case 2: // 'word' format
                wp = (unsigned short*) &buffer[i * BUFWIDE];
                for (j = 0; j < BUFWIDE / 2; j++)
                    linelen += sprintf(outline + linelen, " %04X ", wp[j]);
                break;

            case 4: // 'dword' format
                dp = (unsigned int*) &buffer[i * BUFWIDE];
                for (j = 0; j < BUFWIDE / 4; j++)
                    linelen += sprintf(outline + linelen, "  %08X  ", dp[j]);
                break;

            case 8: // 'qword' format
                qp = (unsigned long long*) &buffer[i * BUFWIDE];
                for (j = 0; j < BUFWIDE / 8; j++)
                    linelen += sprintf(outline + linelen, "    %016llX    ", qp[j]);
                break;

            case 16: // 'octaword'
                qp = (unsigned long long*) &buffer[i * BUFWIDE];
                linelen += sprintf(outline + linelen, "     ");
                linelen += sprintf(outline + linelen, "   %016llX%016llX   ", qp[1], qp[0]);
                linelen += sprintf(outline + linelen, "     ");
                break;
            }
